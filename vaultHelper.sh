...
vaultConfigPath="$HOME/.config/vaultHelper"

TempStorageName="vaultTempStorage"
passwordShareText="passwordshare.txt"
vaultMountText="vaultmount.txt"
vaultPathText="vaultpath.txt"
...
acknowledgementInitialize() {
    ...
    then
        ...
        echo "$VaultPath" > "$1/$vaultPathText" 
    fi

    if [[ ! -f $1/$vaultMountText ]];
    then
        ...
        echo "$VaultMount" > "$1/$vaultMountText"
    fi

    if [[ ! -f $1/$passwordShareText ]];
    then
        if whiptail --title "Vault Password Sharing" \
         --yesno "Do all of your vaults share the same password?" \
         $TerminalLines $TerminalColumns 3>&1 1>&2 2>&3;
        then
            echo "yes" > "$1/$passwordShareText"
        else
            echo "no" > "$1/$passwordShareText"
        fi
    fi
}
...
unlockSelectedVaults() {
    ...
    do
        ...
        then
            whiptail --title "Vault already mounted" \
             --msgbox "Looks like $vaultDestination has already been mounted." \
...
acknowledgementInitialize "$vaultConfigPath"

read -r PasswordShare < "$vaultConfigPath/$passwordShareText"
read -r VaultPath < "$vaultConfigPath/$vaultPathText"
read -r VaultMount < "$vaultConfigPath/$vaultMountText"
...
do
    ...
    then
        whiptail --title "Vault not selected" \
         --msgbox "No vault was selected on the previous page. Please select a vault" \
         $TerminalLines $TerminalColumns
...
whiptail --title "Thank you!" \
 --msgbox "Thanks for using this script ^o^ \n Hope it's made your day a bit easier" \
 $TerminalLines $TerminalColumns